########################################################################################################## 
# POWERSHELL SCRIPT TO EXTRACT MEMORY DUMP CONFIGURATION AND PAGE FILE CONFIGRATION FROM REMOTE SERVERS 
# Version 3.0 .                                                                                        
# Author - Akash Saxena. 
#How to use--
...
#To Run - PS C:\> .\Registry_Settings_Exports.ps1 -InputTextFile .\server.txt                           
##########################################################################################################
Param(
[parameter (Mandatory=$true)] [String] $InputTextFile
)
cls
$servers=(Get-Content $InputTextFile).ToUpper()
$out=".\Output.csv"
$Totaloutput=@()
